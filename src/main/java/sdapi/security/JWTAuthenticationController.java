...
@RestController
@CrossOrigin
public record JWTAuthenticationController(JWTUtil jwtTokenUtil, UserService userService) {
    ...
    public ResponseEntity<?> createAuthenticationToken(@RequestHeader(value = "Authorization", required = false) String authorization, @RequestBody JWTRequest authenticationRequest) throws Exception {
        ...
        User userToAuth = userService.findByEmailAndPassword(authenticationRequest.getEmail(), authenticationRequest.getPassword());
        if (userToAuth == null)
